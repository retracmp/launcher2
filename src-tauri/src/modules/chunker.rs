        ...
        Ok(mut manifest) => {
                            ...
                            loop {
                                ...
                                match retry_result {
                                    ...
                                    Err(retry_err) => {
                                        ...
                                        if retry_err.to_string() == "Download paused" {
                                            loop {
                                                tokio::time::sleep(std::time::Duration::from_millis(100)).await;
                                                let state = util::get_downloading_state().await;
                                                if state.is_cancel_requested(&manifest_id) {
                                                ...
                                                if !state.is_pause_requested(&manifest_id) {
                                                    tokio::time::sleep(std::time::Duration::from_millis(300)).await;
                                                    let state = util::get_downloading_state().await;
                                                    if !state.is_pause_requested(&manifest_id) && !state.is_cancel_requested(&manifest_id) {
                                                        break;
                                                    }
                                                }
                                            }
            ...
            if has_error {
                ...
                if error_msg != "" {
                    ...
                    let _ = handle.emit(
                        "DOWNLOAD_ERROR2",
                        json!({ "manifest_id": manifest_id, "error": "Download cancelled by user." }),
                    );
                    return Ok(false);
                }
            }
